# PowerShell script to automatically set up Windows Task Scheduler for daily TOMORROW's follow-up SMS reminders
...
$smsScriptPath = Join-Path $projectDir "send_tomorrow_sms.ps1"

# Task Scheduler task name
$taskName = "MHOERS Tomorrow Follow-up SMS Reminders"
...
Write-Host "Setting up Task Scheduler for daily TOMORROW's follow-up SMS reminders..." -ForegroundColor Green
...
try {
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description "Automated daily SMS reminders for patients with follow-up appointments scheduled for TOMORROW. Sends at 7:00 AM daily." `
        -Force | Out-Null
